...
class TradingEngine:
    ...
    def __init__(
        ...
        self.last_prices = {}
        self._last_rebalance_ts = None  # persists across step() calls
        self._last_equity = None        # persists across step() calls
    ...
    async def step(self, df_ts: pd.DataFrame, adapt_policy: bool = True) -> None:
        ...
        In production: called by the runner each polling interval with fresh
        recommendations from apply_trading_policy(). Executor safety state
        (peak equity, halt flag, PDT tracking) persists across calls.

        In backtesting: called once per timestamp group by backtest(), which
        applies the policy and sets portfolio_weight before calling here.
        ...
        adapt_policy : bool
            If True (default), call policy.adapt() after each tick so the
            policy continues learning from realized returns. Set False to
            apply the policy exactly as trained.
        """
        ...
        should_rebalance = (
            ...
            or (ts - self._last_rebalance_ts) >= rebalance_interval_s
        )
    ...
    async def backtest(self, adapt_policy: bool = True) -> dict:
        ...
        Calls _restore_intraday_state() once, then delegates each timestamp
        group to step() after applying the policy to produce portfolio weights.
        ...
        # Reset intra-run state so repeated backtest() calls are independent
        ...
        # _last_equity is set by step() whenever adapt_policy=True; fall back
        # to a direct fetch only when adapt_policy=False (no equity was tracked).
        if self._last_equity is not None:
